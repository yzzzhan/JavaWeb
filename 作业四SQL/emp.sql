...
LEFT JOIN projects p ON ep.project_id = p.project_id AND CURDATE() BETWEEN p.start_date AND p.end_date
...
# 33. 查询参与项目最多的部门。
SELECT d.dept_id, d.dept_name, COUNT(DISTINCT ep.project_id) AS project_count
FROM departments d
JOIN employees e ON d.dept_id = e.dept_id
JOIN employee_projects ep ON e.emp_id = ep.emp_id
GROUP BY d.dept_id, d.dept_name
...
SELECT emp_id, first_name, last_name, salary,
       salary * POWER(1.05, YEAR(CURDATE()) - YEAR(hire_date)) AS new_salary
FROM employees;
...
JOIN (
    ...
    FROM employees
    GROUP BY dept_id
) AS dept_min ON e.dept_id = dept_min.dept_id AND e.salary = dept_min.min_salary;
...
SELECT emp_id, first_name, last_name, salary,
       CASE
           ...
           WHEN salary BETWEEN 50000 AND 70000 THEN '中'
           ELSE '高'
       END AS salary_level
FROM employees;
...
SELECT e.emp_id, e.first_name, e.last_name, COUNT(sub.emp_id) AS employee_count
FROM employees e
LEFT JOIN employees sub ON e.emp_id = sub.dept_id AND e.job_title LIKE '%Manager%'
GROUP BY e.emp_id;
...
SELECT d.dept_id, d.dept_name,
       MAX(e.hire_date) AS oldest_employee,
       MIN(e.hire_date) AS youngest_employee
FROM departments d
JOIN employees e ON d.dept_id = e.dept_id
GROUP BY d.dept_id, d.dept_name;
...
SELECT p.project_name, m.first_name, m.last_name, p.start_date, p.end_date
FROM projects p
JOIN (
    SELECT ep.project_id, e.first_name, e.last_name
    FROM employee_projects ep
    JOIN employees e ON ep.emp_id = e.emp_id
    WHERE (ep.project_id, e.salary) IN (
        SELECT ep2.project_id, MAX(e2.salary)
        FROM employee_projects ep2
        JOIN employees e2 ON ep2.emp_id = e2.emp_id
        GROUP BY ep2.project_id
    )
) AS m ON p.project_id = m.project_id;
